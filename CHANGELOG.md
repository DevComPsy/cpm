...
- Added `log_likelihood` and `log_prior` to the fitted output of `cpm.optimisation.Fmin`, `cpm.optimisation.FminBound`, `cpm.optimisation.Minimize`, `cpm.optimisation.Bads` and `cpm.optimisation.DifferentialEvolution` when they are run with `prior=True`, recording the summed log likelihood and the summed log prior density at the optimised parameter values. With a prior, `fun` is the negative summed log posterior density, which on its own cannot be separated into the two terms that post-hoc model selection and goodness-of-fit indices need. The new quantities are recomputed at the optimum by `cpm.core.optimisers.decompose_objective`, which mirrors `objective` step for step, so `fun` is always exactly `-(log_likelihood + log_prior)`. Without `prior=True` the keys are absent, since `fun` is then already the negative summed log likelihood ([#28](https://github.com/DevComPsy/cpm/issues/28))
- Added a `metrics` argument to `cpm.optimisation.Fmin`, `cpm.optimisation.FminBound`, `cpm.optimisation.Minimize`, `cpm.optimisation.Bads` and `cpm.optimisation.DifferentialEvolution`, which evaluates any number of user-supplied goodness-of-fit metrics at the optimised parameter values and records them alongside the fit, so that they appear as columns in `export()`. Metrics are given as a mapping of output names to callables, an iterable of callables named after themselves, or a single callable, and are called with keyword arguments only - `likelihood` and `log_likelihood`, `log_prior`, `predicted`, `observed`, `n`, `k` and `parameters` - following the convention `cpm.optimisation.compare.PenalisedLikelihoods` and the `cpm.optimisation.minimise` loss functions already use, so `PenalisedLikelihoods.BIC`, `PenalisedLikelihoods.AIC` and the loss functions work as metrics unchanged. With `prior=True` the metrics receive the log likelihood rather than the log posterior, so penalised likelihoods stay correctly defined ([#28](https://github.com/DevComPsy/cpm/issues/28))
...
- Fixed `cpm.optimisation.Bads` emitting a `DeprecationWarning` on every GP fit - 119 of them in a two-participant test fit - and being on course to fail outright on Python 3.16. pybads evaluates `~options["gp_fixed_mean"]`, and `~` on a Python `bool` inverts the underlying int rather than negating it, which Python 3.16 removes. `Bads` now hands pybads that option as a NumPy boolean, which negates correctly and is unaffected by the removal. Results are unchanged. The shim is removable once pybads fixes the operator upstream and the version floor is raised ([#88](https://github.com/DevComPsy/cpm/issues/88))
