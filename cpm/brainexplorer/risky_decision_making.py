...
class Scavenger:
    ...
    def metrics(self):
        ...
        for user_id, user_data in grouped_data:
            ...
            user_results["median_RT_win_ambg"] = np.nanmedian(
                user_data[~t_loss & t_abg]["RT"]
            )
            user_results["median_RT_win_NoAmbg"] = np.nanmedian(
                user_data[~t_loss & ~t_abg]["RT"]
            )
            user_results["median_RT_loss_ambg"] = np.nanmedian(
                user_data[t_loss & t_abg]["RT"]
            )
            user_results["median_RT_loss_NoAmbg"] = np.nanmedian(
                user_data[t_loss & ~t_abg]["RT"]
            )
            user_results["diff_RT_win_amg_win_noAmbg"] = (
                user_results["median_RT_win_ambg"]
                - user_results["median_RT_win_NoAmbg"]
            )
            user_results["diff_RT_loss_amg_loss_noAmbg"] = (
                user_results["median_RT_loss_ambg"]
                - user_results["median_RT_loss_NoAmbg"]
            )
            ...
            user_results["mean_points_safe"] = np.nanmean(
                user_data[rsk == 0]["outcome"]
            )
            user_results["mean_points_risky"] = np.nanmean(
                user_data[rsk == 1]["outcome"]
            )

            # mean points for ambigous and non-ambiguous trials
            user_results["mean_points_ambg"] = np.nanmean(user_data[t_abg]["outcome"])
            user_results["mean_points_NoAmbg"] = np.nanmean(
                user_data[~t_abg]["outcome"]
            )

            # mean points for win and loss trials in ambigous and non-ambiguous trials
            user_results["mean_points_win_ambg"] = np.nanmean(
                user_data[~t_loss & t_abg]["outcome"]
            )
            user_results["mean_points_win_NoAmbg"] = np.nanmean(
                user_data[~t_loss & ~t_abg]["outcome"]
            )
            user_results["mean_points_loss_ambg"] = np.nanmean(
                user_data[t_loss & t_abg]["outcome"]
            )
            user_results["mean_points_loss_NoAmbg"] = np.nanmean(
                user_data[t_loss & ~t_abg]["outcome"]
            )
            user_results["diff_points_win_amg_win_noAmbg"] = (
                user_results["mean_points_win_ambg"]
                - user_results["mean_points_win_NoAmbg"]
            )
            user_results["diff_points_loss_amg_loss_noAmbg"] = (
                user_results["mean_points_loss_ambg"]
                - user_results["mean_points_loss_NoAmbg"]
            )
            ...
            user_results["diff_loss_win_risk"] = (
                user_results["loss_risk"] - user_results["win_risk"]
            )
            ...
            user_results["diff_Ambg_NoAmbg_risk"] = (
                user_results["risk_Ambg"] - user_results["risk_NoAmbg"]
            )
            ...
            user_results["rational_win"] = np.sum(
                ev_diff_chosen[~t_loss] >= 0
            ) / np.sum(~t_loss)
            user_results["rational_loss"] = np.sum(
                ev_diff_chosen[t_loss] >= 0
            ) / np.sum(t_loss)
            ...
            user_results["rational_abg"] = np.sum(ev_diff_chosen[t_abg] >= 0) / np.sum(
                t_abg
            )
            user_results["rational_NoAbg"] = np.sum(
            ...
            user_results["rational_safe"] = np.sum(
                ev_diff_chosen[rsk == 0] >= 0
            ) / np.sum(rsk == 0)
            user_results["rational_risky"] = np.sum(
                ev_diff_chosen[rsk == 1] >= 0
            ) / np.sum(rsk == 1)
            ...
            user_results["rational_safe_win"] = np.sum(
                ev_diff_chosen[~t_loss & rsk == 0] >= 0
            ) / np.sum(~t_loss & rsk == 0)
            user_results["rational_safe_loss"] = np.sum(
                ev_diff_chosen[t_loss & rsk == 0] >= 0
            ) / np.sum(t_loss & rsk == 0)
            user_results["rational_risky_win"] = np.sum(
                ev_diff_chosen[~t_loss & rsk == 1] >= 0
            ) / np.sum(~t_loss & rsk == 1)
            user_results["rational_risky_loss"] = np.sum(
                ev_diff_chosen[t_loss & rsk == 1] >= 0
            ) / np.sum(t_loss & rsk == 1)
            ...
            user_results["diff_rational_risky_Ambg_NoAmbg"] = (
                user_results["rational_risky_abg"]
                - user_results["rational_risky_NoAbg"]
            )
            ...
            user_results["diff_rational_safe_risky_NoAmbg"] = (
                user_results["rational_safe_NoAbg"]
                - user_results["rational_risky_NoAbg"]
            )

            # No Brainer Trial Choices
            # count how man wrong choices were made in catch trials
            user_results["nb_incorrect_gain"] = user_data[
                user_data["nbcorrect_gain"] == 0
            ].shape[0]
    ...
    def clean_data(self):
        ...
        self.cleanedresults = self.cleanedresults[
            self.cleanedresults["userID"].isin(user_ids_to_keep)
        ]

        # find user IDs with specific configID to exclude
        user_ids_to_exclude = self.data.loc[
            self.data["config_id"] == 242, "userID"
        ].unique()
        user_ids_to_keep = [
            uid
            for uid in self.data["userID"].unique()
            if uid not in user_ids_to_exclude
        ]
        self.cleanedresults = self.cleanedresults[
            self.cleanedresults["userID"].isin(user_ids_to_keep)
        ]
        ...
        self.cleanedresults = self.cleanedresults[
            self.cleanedresults["nb_incorrect_gain"] < 2
        ]  # exclude participants who failed 2 or more catch trials
