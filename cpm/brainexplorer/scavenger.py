...
class Scavenger():
    ...
    filepath = "/example/2025-01-10_Scavenger.xlsx"
    scavenger = Scavenger(filepath)
    metrics_result = scavenger.metrics()
    codebook_result = scavenger.codebook()
    ...
    def __init__(self, filepath=None):
        """
        Load data to be analyized and turn into Pandas DataFrame
    ...
    def metrics(self):
        ...
        - Catches an error in the loss case logistic regression for one user in which a singular matrix error occurs.
        ...
        for user_id, user_data in grouped_data:
            ...
            for _, trial_data in user_data.iterrows():
                ...
                for field in fields_to_process:
                    if field in trial_data:  
                        ...
                        if isinstance(value, str):
                            normalized_data[field] = [float(x.strip()) for x in value.split(",")]
                        else:                            
                ...
                if len(normalized_data['chosenOption_outcomes_percent']) == 1:
                    ...
                    metrics_data_trial['A_perc'] = normalized_data['chosenOption_outcomes_userVisiblePercent']
                    metrics_data_trial['A_magn'] = normalized_data['chosenOption_outcomes_trialValue']    
                    metrics_data_trial['B_perc'] = normalized_data['unchosenOption_outcomes_userVisiblePercent']
                    metrics_data_trial['B_magn'] = normalized_data['unchosenOption_outcomes_trialValue']
                    ...
                    metrics_data_trial['A_perc'] = normalized_data['unchosenOption_outcomes_userVisiblePercent']
                    metrics_data_trial['A_magn'] = normalized_data['unchosenOption_outcomes_trialValue']
                    metrics_data_trial['B_perc'] = normalized_data['chosenOption_outcomes_userVisiblePercent']
                    metrics_data_trial['B_magn'] = normalized_data['chosenOption_outcomes_trialValue']
                ...
                metrics_data_trial['A_EV'] = np.dot(np.array(metrics_data_trial['A_perc']), np.array(metrics_data_trial['A_magn']))
                metrics_data_trial['B_EV'] = 0.5 * np.array(metrics_data_trial['B_perc']) + 0.5 * np.array(metrics_data_trial['B_magn'])
                metrics_data_trial['B_EV'] = metrics_data_trial['B_EV'][0]
                metrics_data_trial['EV_diff_safe'] = metrics_data_trial['A_EV'] - metrics_data_trial['B_EV']
                if metrics_data_trial['chosen'] == 1:
                    metrics_data_trial['EV_diff_chosen'] = metrics_data_trial['A_EV'] - metrics_data_trial['B_EV']
                elif metrics_data_trial['chosen'] == 2:
                    metrics_data_trial['EV_diff_chosen'] = metrics_data_trial['B_EV'] - metrics_data_trial['A_EV']
                ...
                user_metrics['EV_diff_chosen_all'].append(metrics_data_trial['EV_diff_chosen'])
        ...
        for user_data in all_users_data:
            ...
            user_results['optimal_all'] = np.sum(ev_diff_chosen > 0) / len(ev_diff_chosen)
            user_results['optimal_win'] = np.sum(ev_diff_chosen[~t_loss] > 0) / np.sum(~t_loss)
            user_results['optimal_loss'] = np.sum(ev_diff_chosen[t_loss] > 0) / np.sum(t_loss)
            user_results['optimal_all_abg'] = np.sum(ev_diff_chosen[t_abg] > 0) / np.sum(t_abg)
            user_results['optimal_win_abg'] = np.sum(ev_diff_chosen[~t_loss & t_abg] > 0) / np.sum(~t_loss & t_abg)
            user_results['optimal_loss_abg'] = np.sum(ev_diff_chosen[t_loss & t_abg] > 0) / np.sum(t_loss & t_abg)
            ...
            try:
                ...
                print(f"Singular matrix error for user {user_id} in loss case logistic regression. Skipping.")
                continue
            ...
            self.results = pd.concat([self.results, pd.DataFrame([user_results])], ignore_index=True)
...
if __name__ == "__main__":
    filepath = "2025-01-10_Scavenger_correct.xlsx"
    scavenger = Scavenger(filepath)
    metrics_result = scavenger.metrics()
    codebook_result = scavenger.codebook()
    #metrics_result.to_csv('/Users/lucyk/Downloads/test_metrics_wReg.csv', index=False) 
