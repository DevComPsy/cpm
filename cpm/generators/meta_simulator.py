...
class MetaSignalDetectionSimulator:
    ...
    def __init__(self, wrapper=None, data=None, parameters=None, solver="fminbound", parallel=False, ppt_identifier="ppt"):
        ...
        m.reset(parameters=self.parameters[self.groups[0]], data=self.data, ppt = self.groups[0])
        m.run()
    ...
    def run(self, verbose=True, num_starts=5, num_samples=100):
        ...
        model.reset(parameters=self.parameters[self.groups[0]], data=self.data, ppt = self.groups[0])
        model.run()
    ...
    def export(self):
        ...
        results["ppt"] = np.array([self.num_samples * [ppt] for ppt in self.groups]).flatten()
        ...
        result.rename(columns={f"x_{i}": names[i] for i in range(len(names))}, inplace=True)
